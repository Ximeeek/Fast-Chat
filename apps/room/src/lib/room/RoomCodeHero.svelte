<script lang="ts">
	...
	const isRedacted = $derived(!isOwner && (hoppingEnabled || !roomCode));
...
<div class="w-full">
	{#if isRedacted}
		<div
			class="w-full p-5 sm:p-6 rounded-2xl bg-[#0a0d16]/95 backdrop-blur-md border border-[#1a2233] text-left select-none relative overflow-hidden shadow-[0_4px_25px_rgba(0,0,0,0.5)]"
			role="region"
			aria-label="Room code hidden for security. Active invitation codes are visible exclusively to the room owner."
			title="Room code hidden for security. Active invitation codes are visible exclusively to the room owner."
		>
			<!-- Meta Tag Bar -->
			<div class="flex items-center justify-between gap-2 mb-3">
				<div class="flex items-center space-x-2.5">
					...
					<span class="text-[10px] px-2.5 py-0.5 rounded-full bg-cyan-500/15 text-cyan-400 border border-cyan-500/30 uppercase font-semibold font-mono">
						HOPPING ACTIVE
					</span>
				</div>
			...
			<div class="flex flex-col sm:flex-row sm:items-baseline justify-between gap-2">
				<div class="flex items-center gap-3">
					...
					<span class="text-lg sm:text-xl md:text-2xl font-['JetBrains_Mono',monospace] font-semibold tracking-wider text-zinc-400">
						🔒 Hidden for security
					</span>
				</div>
				<span class="text-[11px] text-zinc-500 font-mono">
					One-time invitation active
				</span>
			</div>
		</div>
	{:else}
			...
			<div class="flex items-center justify-between gap-2 mb-3">
				<div class="flex items-center space-x-2.5">
					...
					<span class="text-[10px] px-2.5 py-0.5 rounded-full bg-blue-500/15 text-blue-400 border border-blue-500/30 uppercase font-semibold font-mono">
						{hoppingEnabled ? 'HOPPING ACTIVE' : 'SECURE ROOM'}
					</span>
				</div>
